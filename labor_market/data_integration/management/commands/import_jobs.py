...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Tecnoempleo (temporalmente desactivado)
        # try:
        ...
        try:
            ...
            jobs = infojobs.scrape_jobs(num_pages=3)
            total_jobs += len(jobs)
            self.stdout.write(self.style.SUCCESS(f'Infojobs import completed: {len(jobs)} jobs imported'))
        except Exception as e:
            self.stdout.write(self.style.ERROR(f'Error importing from Infojobs: {e}'))

        # LinkedIn (temporalmente desactivado)
        # try:
